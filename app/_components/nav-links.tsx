...
export function NavLinks() {
  ...
  return (
    <nav className="grid grid-cols-2 gap-[5px] sm:flex sm:overflow-x-auto sm:gap-[5px] sm:pb-1 lg:block lg:space-y-[5px] lg:overflow-visible lg:pb-0">
      {links.map((link) => {
        const isActive =
          pathname === link.href ||
        ...
        return (
          <Link
            key={link.href}
            href={link.href}
            className={`group flex min-w-0 items-center gap-2.5 rounded-[0.62rem] px-3 py-2.5 text-[0.88rem] font-semibold transition sm:min-w-[132px] sm:shrink-0 sm:text-[0.92rem] lg:min-w-0 ${
              isActive
          ...
          >
            <span
              className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-[0.58rem] transition ${
                isActive
