    ...
    <div className="panel-stack-5">
      ...
      <div className="grid gap-[5px] xl:grid-cols-3">
        {strategies.map((strategy) => (
          <Panel key={strategy.id} className="p-4 sm:p-5">
            <p className="micro-label">Strategy</p>
            <h2 className="mt-3 text-xl font-semibold text-white sm:text-[1.35rem]">{strategy.name}</h2>
            <p className="mt-3 text-sm leading-5 text-slate-300">{strategy.thesis}</p>
            ...
            <div className="signal-divider my-5" />

            <div className="panel-stack-5">
              {strategy.rules.map((rule) => (
                <div
                  key={rule}
                  className="signal-surface-soft rounded-[0.58rem] p-3.5 text-sm leading-5 text-slate-200"
                >
                  {rule}
                </div>
              ))}
